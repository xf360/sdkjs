...
(function(window, document){
    ...
    // глобальные мапы для быстрого поиска 用于快速搜索的全局变量
    var g_map_font_index = {};//字体文件索引
    var g_fonts_streams = [];//字体文件对应的二进制数据

    var isLoadFontsSync = (window["NATIVE_EDITOR_ENJINE"] === true);

    //字体加载类 id为fonts文件夹对应字体文件名
    ...
    {
        console.log('LoadFontFromData---加载字体数据')
    ...
    CFontFileLoader.prototype.LoadFontArrayBuffer = function(basePath)
    {
        ...
        let x=basePath + this.Id//测试
        console.log('CFontFileLoader----LoadFontArrayBuffer:开始请求字体文件',basePath + this.Id)
        ...
        xhr.onload = function()
        {
            console.log(`CFontFileLoader----LoadFontArrayBuffer:字体[${x}]下载完成`,x)
            if (this.status !== 200)
                return this.onerror();
    ...
    //字体信息  sName字体名字 thumbnail缩略图 indexR字体id faceIndexR indexI faceIndexI indexB faceIndexB indexBI faceIndexBI
    ...
    CFontInfo.prototype =
    {
        // начинаем грузить нужные стили 开始加载正确的样式
    ...
    {
        console.log('checkAllFonts---加载字体')
		...
		let infos = window["__fonts_infos"];//字体名称和字体id对应关系
		...
		/////////////////////////////////////////////////////////////////////
		// наш шрифт для спецсимволов 我们的特殊字符字体
		let ascW3 = new CFontFileLoader(ascFontFath);
        ascW3.Status = 0;// -1 - notloaded, 0 - loaded, 1 - error, 2 - loading
		let streams_count = g_fonts_streams.length;
