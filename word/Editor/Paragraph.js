...
function Paragraph(DrawingDocument, Parent, bFromPresentation) {
	console.log('构造段落Paragraph', DrawingDocument, Parent, bFromPresentation)
...
Paragraph.prototype.RecalculateCurPos = function (bUpdateX, bUpdateY, isUpdateTarget, isReturnTarget) {
	console.log('Paragraph重新计算CurPos')
...
Paragraph.prototype.Draw = function (CurPage, pGraphics) {
	console.log('Paragraph.prototype.Draw---绘制段落')
...
//如果该段被另一个用户夹住，则在该段的左侧绘制一个符号
	*/
	...
	//2图片部分：
	//在段落左侧添加一个特殊的符号，用于至少有价值的段落
	//其中一个选项是不要撕开一段（keeplines），不要撕开下一段（keepnext），
	//首页（PageBreakBefore）
